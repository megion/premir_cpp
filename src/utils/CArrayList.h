...
public:
    ...
    CArrayList(size_t _capacity, size_t _capacityIncrease = 1)
        : length(0)
        , capacity(_capacity)
        , capacityIncrease(_capacityIncrease)
        , typeSizeof(sizeof(T))
        , array(nullptr) {
        if (capacity == 0) {
            array = nullptr;
    ...
    CArrayList(const T *arr, size_t len)
        : length(len)
        , capacity(len)
        , capacityIncrease(1)
        , typeSizeof(sizeof(T)) {
     ...
     */
    void setSize(size_t size) { length = size; }

    T *getArray() const { return array; }

    size_t getTypeSizeof() const { return typeSizeof; }
    ...
    CArrayList<T> &operator=(CArrayList<T> &&list) {
        /*
         * TODO: need is array nullptr if not we should clear it before assign
         * removeAll(); // ? free(): double free detected in tcache 2
         */
    ...
    // [] index operator
    T &operator[](const size_t &index) const { return array[index]; }
    ...
    class Iterator {
    public:
        Iterator(T *_arr, size_t _pos) : arr(_arr), pos(_pos) {}

        bool operator!=(const Iterator &other) const {
            return pos != other.pos;
        }

        T &operator*() const { return arr[pos]; }
    ...
    void push(T &&value) {
        ...
        initializeArrayMemory(newLength);
        // should exist move assignment operator
    ...
    void write(size_t position, const T &value) { write(position, &value, 1); }
