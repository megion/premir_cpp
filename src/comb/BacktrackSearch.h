...
class BacktrackSearch {
    public:
    BacktrackSearch() : finished(false), maxCandidates(MAX_CANDIDATES) {}
    ...
    /**
     * solutionsVector - vector of solutions, 'arr[i]' store 'true' if current
     *set contains element 'i'
     * treeDepth - deep of tree (initialize with 0)
     *
     * На каждом этапе алгоритма перебора с возвратом мы пытаемся расширить
     *данное
     * частичное решение a = (a[1], a[2], ..., a[k]) добавляя следующий элемент
     *в конец
     * последовательности. После расширения последовательности нам нужно
     *проверить, не содержит
     * ли она полного решения.
     *
     * При переборе с возвратом создается дерево, в котором каждая вершина
     *представляет
     * частичное решение. Если узел Y создан в результате перехода от узла X, то
     *эти узлы
     * соединяются ребром. Такое дерево частичных решений предоставляет
     *альтернативный
     ...
     */
    void backtrack(T* const solutionsVector, size_t treeDepth,
        ...
        if (isSolution(solutionsVector, treeDepth, solutionSize)) {
            ...
            constractCandidates(solutionsVector, currTreeDepth, candidates,
                                numCandidates, solutionSize);
            //				std::cout << "+++ start iterate candidates"  <<
            //std::endl;
            ...
            //				std::cout << "--- end iterate candidates"  <<
            //std::endl;
        }
    }

    void printSolutionsVector(T* const solutionsVector,
                              const size_t& solutionSize) {
     ...
     */
    virtual bool isSolution(T* const solutionsVector, size_t treeDepth,
                            const size_t& solutionSize) = 0;

    /**
     * process full solution
     */
    virtual void
    processSolution(T* const solutionsVector, size_t treeDepth,
                    const size_t& solutionSize,
                    utils::RDMatrix<bool, size_t>* solutionResults) = 0;

    /**
     * write full set off candidates to candidates array for 'treeDepth'
     * position of vector 'solutionsVector'
     */
    virtual void constractCandidates(T* const solutionsVector, size_t treeDepth,
                                     T* const candidates, size_t& numCandidates,
                                     const size_t& solutionSize) = 0;

    /**
     * data modification
     */
    virtual void makeMove(T* const solutionsVector, size_t treeDepth,
                          const size_t& solutionSize) = 0;

    /**
     * data modification
     */
    virtual void unmakeMove(T* const solutionsVector, size_t treeDepth,
                            const size_t& solutionSize) = 0;

    protected:
    bool finished; // all solutions have been found
    const static size_t MAX_CANDIDATES =
        1000; // max count candidates for next position
