...
class PermutationBacktrackSearch : public BacktrackSearch<size_t> {
    public:
    PermutationBacktrackSearch() : BacktrackSearch<size_t>::BacktrackSearch() {}

    PermutationBacktrackSearch(size_t _maxCandidates)
        : BacktrackSearch<size_t>::BacktrackSearch(_maxCandidates) {}

    bool isSolution(size_t* const solutionsVector, size_t treeDepth,
                    const size_t& solutionSize) {
        return treeDepth == (solutionSize - 1);
    }

    void processSolution(size_t* const solutionsVector, size_t treeDepth,
                         const size_t& solutionSize,
    ...
    void constractCandidates(size_t* const solutionsVector, size_t treeDepth,
                             size_t* const candidates, size_t& numCandidates,
                             const size_t& solutionSize) {
        ...
        for (size_t i = 0; i < treeDepth; i++) {
            //				std::cout << "k " << k << " i " << i <<" set to true
            //inPerm[" << solutionsVector[i] << "]" << std::endl;
    ...
    void makeMove(size_t* const solutionsVector, size_t treeDepth,
                  const size_t& solutionSize) {}

    void unmakeMove(size_t* const solutionsVector, size_t treeDepth,
                    const size_t& solutionSize) {}
};
}
