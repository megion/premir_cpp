...
class LinkedHashMap {
    ...
    class EntryComparator {
         ...
         */
        virtual bool equals(const T* entry, unsigned int keyHash,
                            const D* keyData) const = 0;
        /*
    ...
    LinkedHashMap(size_t initial_size, EntryComparator* _comparator)
        : size(0), comparator(_comparator)
    {
        unsigned int _size = HASHMAP_INITIAL_SIZE;

        /* calculate initial table size and allocate the table */
        initial_size =
            (unsigned int)((uint64_t)initial_size * 100 / HASHMAP_LOAD_FACTOR);
    ...
    class MapInterator {
        public:
        // hashmap_iter_init
        MapInterator(LinkedHashMap<T, D>* _map)
            : map(_map), nextEntry(nullptr), tablepos(0)
        {
        }
     ...
     * hashmap_get or hashmap_get_from_hash
     */
    T* getEntry(const T* key) const { return *findEntryPtr(key); }
    ...
    void alloc_table(unsigned int _size)
    {
        ...
        grow_at =
            (unsigned int)((uint64_t)tablesize * HASHMAP_LOAD_FACTOR / 100);
