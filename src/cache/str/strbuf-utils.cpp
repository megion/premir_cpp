...
// void addUniqueAbbrev(StringBuffer *sb, const unsigned char *sha1, int
// abbrev_len) {
...
ssize_t strbuf_read_file(StringBuffer* sb, const char* path, size_t hint)
{
    ...
    ssize_t rlen = strbuf_read(sb, fd, hint);
    close(fd);
    if (rlen < 0) {
        return -1;
    }
...
void strbuf_humanise_bytes(StringBuffer* sb, off_t bytes)
{
    if (bytes > 1 << 30) {
        ...
        sb->addf("%u.%2.2u MiB", x >> 20, ((x & ((1 << 20) - 1)) * 100) >> 20);
    } else if (bytes > 1 << 10) {
        int x = bytes + 5; /* for rounding */
        sb->addf("%u.%2.2u KiB", x >> 10, ((x & ((1 << 10) - 1)) * 100) >> 10);
    } else {
...
void strbuf_add_absolute_path(StringBuffer* sb, const char* path)
{
    ...
    if (!is_absolute_path(path)) {
        ...
        if (pwd && strcmp(pwd, cwd) && !stat(cwd, &cwd_stat) &&
            (cwd_stat.st_dev || cwd_stat.st_ino) && !stat(pwd, &pwd_stat) &&
...
void strbuf_addftime(StringBuffer* sb, const char* fmt, const struct tm* tm)
{
    ...
    sb->grow(hint);
    size_t newlen =
        strftime(sb->buf + sb->len, sb->getAlloc() - sb->len, fmt, tm);
