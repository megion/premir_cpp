...
class StringList {
    ...
    int addEntry(int insert_at, const char* string) {
        int exact_match = 0;
        unsigned int index =
            insert_at >= 0 ? insert_at : getEntryIndex(string, &exact_match);
    ...
    ListItem* insert(const char* string) {
    ...
    ListItem* lookup(const char* string) {
    ...
    /*
     * Can remove duplicate items from a sorted list
     * You shoul sort list before run this function.
    ...
    class Iterator {
        public:
        Iterator(ListItem* _arr) : arr(_arr) {}

        bool operator!=(const Iterator& other) const {
            return arr != other.arr;
        }

        ListItem& operator*() const { return *arr; }
    ...
    Iterator end() const { return Iterator(items + nr); }
    /////////////////////////////////

    /* Use this function or the macro below to iterate over each item */
    typedef bool (*string_list_each_func_t)(struct ListItem*, void*);
    ...
    ListItem* appendNodup(char* string) {
        struct ListItem* retval;
        allocGrow(nr + 1);
        retval = &items[nr++];
    ...
    ListItem* append(const char* string) {
        return appendNodup(strdup_strings ? strdup(string) : (char*)string);
    }

    void sort() {
        my_qsort_s(items, nr, sizeof(*items), cmp_items, (void*)cmp);
    }

    ListItem* unsortedLookup(const char* string) {
    ...
    int splitInPlace(char* string, int delim, int maxsplit) {
        ...
        if (strdup_strings) {
            LOG(ERR, "internal error in splitInPlace(): strdup_strings must "
                     "not be set");
        }
        for (;;) {
            count++;
    ...
    unsigned int size() { return nr; }

    ListItem* getItems() { return items; }

    // [] index operator
    ListItem& operator[](const size_t& index) const {
        return items[index];
    }
    ...
    void reallocArray() {
        size_t amount = sizeof(ListItem) * alloc;
        ListItem* newItems =
            (ListItem*)std::realloc(items, amount);
    ...
    static int item_is_not_empty(ListItem* item, void* unused) {
        return *item->string != '\0';
    }

    static int cmp_items(const void* a, const void* b, void* ctx) {
        compare_strings_fn cmpfn = (compare_strings_fn)ctx;
        const ListItem* one = (ListItem*)a;
        const ListItem* two = (ListItem*)b;
