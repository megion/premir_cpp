...
test_entry* alloc_test_entry(int hash, const char* key, int klen,
                             const char* value, int vlen)
{
    test_entry* entry =
        (test_entry*)std::malloc(sizeof(test_entry) + klen + vlen + 2);
    if (entry == nullptr) {
...
void perf_hashmap(unsigned int method, unsigned int rounds)
{
    ...
    test_entry** entries =
        (test_entry**)std::malloc(TEST_SIZE * sizeof(test_entry*));
    unsigned int* hashes =
        (unsigned int*)std::malloc(TEST_SIZE * sizeof(unsigned int));
    TestEntryComparator comparator;
    cache::LinkedHashMap<test_entry, test_entry_data> allValuesMap(0,
                                                                   &comparator);
...
void test_add_value()
{
    ...
    test_entry* old =
        map.put(alloc_test_entry(cache::strhash(p1), p1, l1, p2, l2));
    assert(old != nullptr);
    std::free(old);
    assert(map.getSize() == 2);

    // try remove
    test_entry* lookupEntry =
        alloc_test_entry(cache::strhash(p1), p1, l1, p2, l2);
...
void test_map_iteration()
{
    ...
    test_entry* lookupEntry =
        alloc_test_entry(cache::strhash(p1), p1, l1, "", 0);
